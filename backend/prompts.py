...
9) 반드시 아래 [출력 스키마]의 JSON 객체 하나로만 응답한다. 그 외 텍스트·코드블록 금지."""
...
2) 기존 facts와 대조한다.
   - 새롭거나 보완하는 사실 → facts에 추가/갱신한다.
   - 기존 사실과 '명백히 모순'되면(예: 기존 first_child=true 인데 '둘째'라고 함) →
     facts는 기존 값 그대로 두고(절대 바꾸지 말 것) conflict로 보고한다.
     사용자 확인을 받은 뒤(다음 턴) 반영한다.
   - 모순이 없으면 conflict는 null.
3) conflict.question / conflict.options 작성 규칙:
   ...
   - options는 사용자가 그대로 고를 자연스러운 보기. 예) ["첫 아이가 맞아요", "둘째가 맞아요"]
4) 한 턴에 conflict는 최대 1개만 보고한다(가장 중요한 모순).
